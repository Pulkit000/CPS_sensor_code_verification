...
if __name__ == '__main__':
    ...
    utime.sleep_ms(80) # delay after write command
    read_measurement=bytearray(8) # value of temperature and humidity measurement ontained in bytearray format
    i2c_obj.read(AHt20_SLAVE_ADDR, b'', 0, read_measurement, 8,0)
    ...
    humidity_raw=(my_data[2]<<16|my_data[3]<<8|my_data[4]<<4) # bit manipulation to get raw humidity data in digital format
    temperature_raw=(my_data[5]<<12|my_data[6]<<8|my_data[7]) # bit manipulation to get raw temperature data in digital format
    ...
    for i in range(10):
        print("the  humidity is",humidity_new) # calibrated humidity data
        print("the  temperature is ",temperature_new) # calibrated temperature data
